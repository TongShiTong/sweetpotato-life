    ...
    sharePosteCanvas: function(avaterSrc, codeSrc, imgInfo) {
      ...
      query.select('#canvas-container').boundingClientRect(function(rect) {
        ...
        if (that.data.productname) {
          ...
          let [contentLeng, contentArray, contentRows] = that.textByteLength((that.data.productname).substr(0, 46), CONTENT_ROW_LENGTH);
          ctx.setTextAlign('left');
        ...
        if (that.data.qita) {
          ...
          let [contentLeng, contentArray, contentRows] = that.textByteLength((that.data.qita).substr(0, 26), CONTENT_ROW_LENGTH);
          ctx.setTextAlign('left');
          ctx.setFillStyle('#000');
          ctx.setFontSize(18);
          let contentHh = 22 * 1;
          for (let m = 0; m < contentArray.length; m++) {
            ctx.fillText(contentArray[m], 15, 35 + contentHh * m);
          }
        }
